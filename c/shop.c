...
#define NUM_PRODUCTS 24 // Number of products in the shop (in stock.csv file)
...
#define PIECES_LOW                                                             \
  1 // Lowest number of items of a particular item type in a customer order
#define PIECES_HIGH                                                            \
  100 // Highest number of items of a particular item type in a customer order
#define EURO "€" // Euro symbol
#define REORDER_THRESHOLD                                                      \
  5 // Shopkeeper reorders item if stock is below this threshold

/* Paths to data files */
char STOCK_PATH[] =
    "../stock.csv"; // Stock, quantities, prices, plus initial shop cash
char NAMES_PATH[] = "../names.list"; // List of 200 names for customers
char FACES_PATH[] = "../faces";      // 19 ASCII art faces for customers
char CUSTOMERS_PATH[] =
    "../customers.csv"; // Saved andomly generated customers go here
...
// Wait for user to press enter or quit
bool cont_or_quit() {
...
struct Customer generate_customer(struct Shop shop, int b_low, int b_high,
                                  int i_low, int i_high, int p_low, int p_high)

{
  struct Customer customer;
  // Name and face are malloc'd in get_name() and get_face()
  // So need to be freed when customer is finished transaction
  char *name = get_name();
  char *face = get_face();
  customer.name = name;
  customer.face = face;

  // Generate random budget and number of items
  customer.budget = (double)(rand() % (b_high - b_low + 1) + b_low);
  int num_items = (rand() % (i_high + 1 - i_low) + i_low);

  // Keep track of shopping list size
  customer.sl_index = num_items;
  // For each item in shopping list
  for (int i = 0; i < num_items; i++) {
    // Get a random item from the shop's stock
    int index = rand() % shop.index;
    // generate a random quantity of that item
...
struct Customer *load_customers(struct Shop shop, char *path,
                                int *num_customers) {
  FILE *fp;          // filehandle
  char *line = NULL; // line buffer
  size_t len = 0;    // line length
  ...
  while ((read = getline(&line, &len, fp)) != -1) {
    ...
    if (id_flag) {
      ...
      if (cust_idx > 0) {
        customers =
            realloc(customers, sizeof(struct Customer) * (cust_idx + 1));
      }
...
// Create string for single product -- used to build receipts, etc
char *stringify_product(struct Product product) {
  char *string = malloc(sizeof(string) * strlen(LINE_LONG) + 1);
  snprintf(string, strlen(LINE_LONG), "%-15s%7.2f", product.name,
           product.price);
  return string;
}
...
char *stringify_list(struct ProductStock *itemlist, int max_idx, bool total) {
  // The separator lines and headers are dependent on whether row totals are
  // required
  char *line = LINE;
  ...
  int offset =
      snprintf(string, strlen(line) * 3, "%s%s\n%s", line, header, line);
...
char *stringify_shop(struct Shop shop) {
  // Each line is strlen(LINE) chars long. We need 1 line for each stock item
  // Plus five lines for the header and footer
...
double total_bill(struct Customer customer) {
  ...
  for (int i = 0; i <= customer.sl_index - 1; i++) {
    total += (customer.shoppingList[i].product.price *
...
char *
stringify_bill(struct Customer customer) { // LINE_LONG = num of chars per row;
  // allocate memory for number of rows in customer shopping list plus 5 for
  // lines, header and footer
  char *string =
      malloc(sizeof(string) * strlen(LINE_LONG) * (customer.sl_index + 5));
  // Stringify the customer's shopping list with row totals
  char *list_string =
      stringify_list(customer.shoppingList, customer.sl_index, true);
  snprintf(string, sizeof(string) * strlen(LINE_LONG) * (customer.sl_index + 5),
           "%sTotal Cost: %37.2f\n%s", list_string, total_bill(customer),
           LINE_LONG);
...
char *stringify_customer(struct Customer customer) {
  char *string =
      malloc(sizeof(string) * 400 + (strlen(LINE_LONG) * customer.sl_index) +
             (strlen(LINE_LONG) * 5));
  char *bill_string = stringify_bill(customer);
  snprintf(string,
           sizeof(string) * 400 + (strlen(LINE_LONG) * customer.sl_index) +
               (strlen(LINE_LONG) * 5),
           "%s\nName: %s\nBudget:%7.2f\n%s", customer.face, customer.name,
           customer.budget, bill_string);
...
void restock(struct Shop *shop, char *product) {
  for (int i = 0; i < shop->index; i++) {
    // Find the product in the shop stock
    if (strcmp(shop->stock[i].product.name, product) == 0) {
      ...
      shop->cash -= shop->stock[i].product.price *
                    (shop->stock[i].product.max_quantity - current_quantity);
    }
  }
}

// Test to see if any item stocks are below reorder threshold and return array
// of those items
struct Product *check_stock(struct Shop *shop, int reorder_threshold,
                            int *size) {
  // Count number of items below reorder threshold
  int count = 0;
  for (int i = 0; i < shop->index; i++) {
  ...
  // Use count to allocate memory for the array of items
  struct Product *out_of_stock = malloc(sizeof(struct Product) * count);

  int index = 0; // Keep track of location in out of stock array
  // Restock items below reorder threshold
  for (int i = 0; i < shop->index; i++) {
    if (shop->stock[i].quantity < reorder_threshold) {
...
double shop_visit(struct Shop *shop, struct Customer customer) {
  ...
  if (cont_or_quit() == true) {
    // free customer's name & face before they go out of scope
    free(customer.name);
    free(customer.face);
    main_menu(shop);
  }
  ...
  struct Product *out_of_stock =
      check_stock(shop, REORDER_THRESHOLD, num_out_of_stock);
  if (cont_or_quit() == true) {
    main_menu(shop);
  }
  clear_console();
  ...
  // Restock items in out of stock array
  ...
  // out of stock variables no longer needed
  free(out_of_stock);
  ...
  // Calculate and return take
  double take = shop->cash - cash;
  return take;
}

// auto mode -- will run until user quits
void auto_mode(struct Shop *shop) {
  while (1) {
    struct Customer customer =
        generate_customer(*shop, BUDGET_LOW, BUDGET_HIGH, ITEMS_LOW, ITEMS_HIGH,
                          PIECES_LOW, PIECES_HIGH);
    shop_visit(shop, customer);
  }
}

// Preset mode -- loads customers from csv file in CUSTOMERS_PATH
void preset_mode(struct Shop *shop) {
  // Keep track of the total take for the run of customers
  double total_take = 0;
  // Need to pass empty pointer to load_customers, so size of array is available
  // later
  int *num_customers = malloc(sizeof(int));
  // Get an array of customer structs
  struct Customer *customers =
      load_customers(*shop, CUSTOMERS_PATH, num_customers);

  // send each cutomer on a shop visit
  for (int i = 0; i < *num_customers; i++) {
    // shop_visit returns the take from that visit
  ...
  if (total_take == 0) {
    ...
    printf("%s %s %.2f %s\n\n", "At least I made ", EURO, total_take,
           "profit.");
  } else {
    printf("%s %s%.2f\n\n", "And after all that I lost", EURO,
           fabs(total_take));
  }

  // Don't need the customers anymore
  free(customers);
  free(num_customers);
  cont_or_quit();
}

// Live mode -- will run until user runs out of money or quits
bool live_mode(struct Shop *shop, bool seen) {
  ...
  // Welcome customer -- if seen is true get a different message
  clear_console();
  printf("%s\n", shopkeeper);
  if (seen) {
  ...
  while (1) {
    if (scanf("%lf", &budget) <= 0) {
      ...
      printf("%s\n", "Well it's not much but hopefully you'll be able to find "
  ...
  while (1) {
    ...
    if (budget < min_price) {
      if (budget == 0) {
        ...
        printf("%s %s%.2f %s\n%s %s%.2f.\n", "You've only got", EURO, budget,
               "left.", "The cheapest item I have costs", EURO, min_price);
      }
    ...
    // Customer can enter number or name
    char product_name[20];
    char q; // for sscanf 'Q' detector
    int product_num;
    struct Product product;
    flush_stdin();
    ...
    // Now that we know what the customer wants we can request quantity from
    // customer
...
// Generate customers with random attributes and write to file
void generate_customers(int num_customers, struct Shop shop, double b_low,
                        double b_high, int i_low, int i_high, int p_low,
                        int p_high, char *names_path, char *customers_path) {
  ...
  for (int i = 0; i < num_customers; i++) {
    ...
    struct Customer customer =
        generate_customer(shop, b_low, b_high, i_low, i_high, p_low, p_high);
...
void display_menu() {
  clear_console();
  char line[] = "-------------------";
  char *menu_items[] = {"Auto Mode", "Preset Mode", "Live Mode",
                        "Generate Customers"};
...
void main_menu(struct Shop *shop) {
  ...
  while (1) {
    ...
    case '4': {
      generate_customers(5, *shop, BUDGET_LOW, BUDGET_HIGH, ITEMS_LOW,
                         ITEMS_HIGH, PIECES_LOW, PIECES_HIGH, NAMES_PATH,
                         CUSTOMERS_PATH);
      break;
    }
    case 'x': {
...
int main(void) {
  ...
  // Splash screen -- only appears once
  clear_console();
  printf("%s\n", "-------------------");
  printf("%s\n\n", "Shop Simulator 2000");
  printf("%s\n\n", shopkeeper);
